...
| Backup & Restore | `backup` | `GET /api/backup/appliance`, `/api/backup/secrets`; `POST /backup/appliance{,/upload,/upload-settings}`, `DELETE /backup/appliance/uploads/{id}`, `POST /backup/appliance/{name}/{validate,restore}`, `POST /backup/secrets{,/…/validate,/…/restore}` | Create local/portable backup; upload+preview dropzone (`.apdnsbak`/`.tar.gz` — **V1.1.1 `.tar.gz` compatibility is a hard requirement**); retention settings; restore preview (recommended/all/none selection, conflict policy, sensitive-category ack, typed-filename confirm — **mandatory pre-restore safety backup is a locked requirement**) | Appliance backups, Uploaded archives, Safety backups, Restore jobs, Secret backups grids | **in progress** -- `internal/backup` (new, native Go): `VACUUM INTO` snapshots of every native-Go-owned table (blocklists, Local DNS, DNS Settings/upstreams, Clients & groups, Filters/custom rules, Clients & Access policy layers/networks -- deliberately excludes `sessions` and `schema_migrations`) into a manifest-tagged tar archive; list/create/upload/preview/delete; restore is genuinely transactional (`ATTACH DATABASE` + a single transaction -- every native table is replaced together or none are) and always takes a mandatory pre-restore safety backup first, itself listed like any other backup so nothing is silently discarded. Archive-bomb defenses: a compressed-size cap (500MB) and an expanded-size cap (2GB), both enforced before any extraction happens. **Not built, and explicitly disclosed in the format's own doc comment**: no byte-compatibility with Python's encrypted `.apdnsbak` (this is a separate container/manifest format with no shared crypto -- a real bridge to either legacy format is future work, not attempted here), no V1.1.1 `.tar.gz` import (a real legacy-format parser, out of scope for this slice), no per-category selective restore (current restore replaces all native tables together, not a `recommended/all/none` per-category selection), no secrets/certs in the archive (this control plane doesn't own any secrets store yet), no retention policy or scheduled backups, no Secret Backups grid (`/api/backup/secrets` not built), no conflict-policy options, no async Restore Jobs history (restore is synchronous, not queued) | **in progress** -- `BackupView.svelte`: create/upload cards, a DataGrid of stored backups (filename/created/reason/size), a per-row typed-filename-confirmation restore dialog (Confirm restore stays disabled until the exact filename is typed -- same locked-confirm pattern as Upstreams' last-enabled-profile guard), delete. Discloses the `.apdnsbak`/`.tar.gz` gap in-page. No restore-preview UI (recommended/all/none selection, conflict policy, sensitive-category ack), no Secret Backups grid, no Restore Jobs grid | done for what's built (8 dedicated Chromium checks -- create adds a real row, restore-confirm is correctly gated on an exact filename match at every stage, restoring reports the automatic safety backup by name and that backup itself appears in the list afterward -- plus the full viewport/theme sweep at `/ui/backup`; 12 `acceptance.py` checks incl. a real file-upload round trip, corrupt-archive rejection, and proving restore actually changes data back and takes a safety backup first) | not started |
