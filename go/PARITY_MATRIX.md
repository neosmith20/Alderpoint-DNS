...
| Clients | `clients` | `GET /api/clients`, `/api/discovery/observed-clients`, `/api/discovery/status`, `/api/groups`; `POST /api/clients`, `/clients/{id}/identifiers`, `/clients/{id}/groups`, `/discovery/observed-clients/{ip}/promote`; `DELETE /discovery/observed-clients/{ip}` | Metric strip; "Manage Client" (not "Promote" — locked owner decision) + Forget; Create Managed Client form; Explain link → Policies | Observed Clients grid, Managed Clients grid | — | none | unmanaged/client-N badges, empty | **in progress** -- `internal/clients` (new, native Go schema/CRUD, field-matched against `control_db.py`'s clients/client_identifiers/client_groups/client_group_members tables): create client, create group, add identifier (ipv4/ipv4_cidr/ipv6/ipv6_cidr/clientid, format-validated via `net/netip`), assign to group. **Not started**: Observed Clients/discovery (Python-owned live data, same class of problem as Dashboard's analytics -- needs its own compatibility-boundary decision), "Manage Client"/Forget workflow (that's specifically promoting an *observed* client, blocked on the same discovery gap), per-client policy assignment (shared policy-layer infrastructure, see the "Clients & Access" row below and the Sequencing note) | **in progress** -- `ClientsView.svelte`: add-client and add-group forms, DataGrid-backed client table with identifier/group chips, inline add-identifier and assign-group forms per row. Discloses its own Managed-Clients-only scope in-page. No Observed Clients UI, no metric strip, no policy editor | done for everything built (6 Chromium checks incl. full viewport/theme sweep; a real bug the suite caught before shipping -- Go's `ListGroups`/`ListClients` returned a nil slice on an empty table, which JSON-marshals to `null` and crashed the shared DataGrid on `null.length` -- fixed at the service-layer source plus a defensive `rows ?? []` in DataGrid itself, with regression tests in both Go packages) | not started |
