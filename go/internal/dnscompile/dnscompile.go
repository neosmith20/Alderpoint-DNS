...
type TransportSettings struct {
	...
	DoqEnabled bool
	DoqPort    int
	// Doh3Enabled/Doh3Port: DNS-over-HTTP/3. Unlike DoH, this real
	// installed dnsdist's addDOH3Local accepts no path/urls option at
	// all (every key of that shape was confirmed, live, to be silently
	// ignored with an "Unknown key" warning) -- it always serves the
	// implementation's own fixed default path, so Strong ClientID's
	// per-client DoH-path binding (see ClientIdentity/HTTPPathRule
	// below) cannot extend to DoH3 in this dnsdist version. Enable/port
	// only, honestly, rather than emitting a path option the real
	// binary would just ignore.
	Doh3Enabled bool
	Doh3Port    int

	// DNSCrypt: real as of internal/dnscryptprovision (provider/resolver
	// key material generated by the real dnsdist binary itself). Unlike
	// TLSCertPath/TLSKeyPath (shared by DoT/DoH/DoQ/DoH3), DNSCrypt's
	// cert/key are its own real files, since they're a distinct signed
	// artifact (a DNSCrypt cert, not an X.509 one) generated and rotated
	// independently -- see internal/dnstransports.Service.RotateDNSCrypt.
	DNSCryptEnabled      bool
	DNSCryptPort         int
	DNSCryptProviderName string
	DNSCryptCertPath     string
	DNSCryptKeyPath      string
}
...
func CompileDnsdist(in Input) (string, error) {
	...
	if in.Transports.Doh3Enabled {
		if in.TLSCertPath == "" || in.TLSKeyPath == "" {
			return "", errf("DoH3 is enabled but no TLS cert/key path is configured")
		}
		host := hostOf(in.ListenAddress)
		w(`addDOH3Local(%s, {%s}, {%s}, {`, luaString(fmt.Sprintf("%s:%d", host, in.Transports.Doh3Port)), luaString(in.TLSCertPath), luaString(in.TLSKeyPath))
		w("  reusePort=true")
		w("})")
		w("")
	}
	if in.Transports.DNSCryptEnabled {
		if in.Transports.DNSCryptCertPath == "" || in.Transports.DNSCryptKeyPath == "" {
			return "", errf("DNSCrypt is enabled but no provider identity has been provisioned yet (rotate first)")
		}
		host := hostOf(in.ListenAddress)
		w(`addDNSCryptBind(%s, %s, %s, %s)`,
			luaString(fmt.Sprintf("%s:%d", host, in.Transports.DNSCryptPort)),
			luaString(in.Transports.DNSCryptProviderName), luaString(in.Transports.DNSCryptCertPath), luaString(in.Transports.DNSCryptKeyPath))
		w("")
	}
