...
func main() {
	...
	dnsRuntimeHealthCheckTimeoutSeconds := flag.Int("dns-runtime-health-check-timeout-seconds", 60, "how long a post-promote health check retries the real marker query before giving up and rolling back. A real live defect found during a durability pass: the original hardcoded 5s default was too short for a realistic blocklist-heavy deployment -- a compiled dnsdist config carrying ~855k blocked domains (a single large inline Lua table) measured ~5s just to pass --check-config alone. A first fix raised this to 30s, but direct timing instrumentation of a real live promote under real production restart conditions showed dnsdist's own startup log only reporting \"Listening on Do53 frontend\" about 4 seconds AFTER a 30s health-check budget had already given up and triggered rollback. 60s leaves real margin above that measured figure. (See internal/hostagentd/ops_dnsruntime.go's applyDefaults for the same default and the full diagnostic trail; this flag's default must stay in sync with it.)")
