...
| Configuration import/export via UI or API | Not present in AdGuard's API either (config-file-only) | Import and Migration page: AdGuard Home migration (YAML upload or read-only API connection), Pi-hole text/list import, Alderpoint DNS-native JSON export/import, plus CSV/XLSX/hosts/BIND-zone/Alderpoint DNS-CSV import for Local DNS, with column mapping, preview, conflict resolution, backup, and rollback | `app/importer.py`, `web/templates/import_migration.html` (`/import`) | complete | none — `tests/test_importer.py` | Alderpoint DNS now goes further than AdGuard here (which has no import/export API at all): a real translation layer for filters/rewrites/clients/upstream resolvers, not just Local DNS CSV. Full appliance backup/restore remains `scripts/backup.sh`/`restore.sh` and the native Backup page. |
