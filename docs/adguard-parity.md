...
| AdGuard feature | Where in AdGuard | BindGuard equivalent | Implementation location | Status | Missing tests | Notes |
|---|---|---|---|---|---|---|
| Recursive vs. forwarding mode | AdGuard can resolve recursively itself or forward to `upstream_dns` | BIND is a forwarding-only validating resolver; managed upstreams are rendered into a local dnsdist upstream pool used by BIND | `app/upstream_dns.py`, generated `/var/lib/bindguard/compiled/bind/upstream-forwarders.conf`, generated `/var/lib/bindguard/compiled/dnsdist/upstream-forwarder.conf` | complete (as forwarder) / intentionally not applicable (as full recursive-from-root) | none — `tests/test_bind_backend.sh`, `tests/test_upstream_dns.py` | BIND remains the recursive/policy point; dnsdist adds plain/DoT/DoH upstream transport support on a loopback listener. There is no UI toggle to switch to root-based recursion. |
...
| Bootstrap DNS servers (resolve upstream hostnames) | Settings → DNS settings → Bootstrap DNS servers | Per-resolver bootstrap IPs for DoH/DoT hostnames | `app/upstream_dns.py`, `web/templates/dns_settings.html` | complete | none — `tests/test_upstream_dns.py::test_doh_upstream_requires_https_and_bootstrap_for_hostname` | BindGuard stores bootstrap IPs with the resolver and uses them as dnsdist backend addresses while validating the TLS subject name against the hostname. |
| Fallback DNS servers (used when primary upstreams fail) | Settings → DNS settings → Fallback DNS servers | Multiple enabled upstream resolvers with explicit order, selected by dnsdist's `firstAvailable` policy | `app/upstream_dns.py::render_dnsdist_upstreams` | partial | none — `tests/test_upstream_dns.py::test_deploy_multiple_enabled_resolvers` | BindGuard does not model a separate "fallback" tier, but ordered multiple upstreams provide redundancy. |
...
| AdGuard feature | Where in AdGuard | BindGuard equivalent | Implementation location | Status | Missing tests | Notes |
|---|---|---|---|---|---|---|
| Cache size / TTL min / TTL max | Settings → DNS settings → Cache configuration | BIND recursive-cache tuning: explicit `max-cache-size` (defaulted from VM memory), `min-cache-ttl`/`max-cache-ttl`, `min-ncache-ttl`/`max-ncache-ttl`, `recursive-clients`, all admin-editable | `app/dns_cache.py`, `web/templates/dns_cache.html` (`/dns-cache`) | complete | none — `tests/test_dns_cache.py` (validation, rendering, deploy/rollback) | BindGuard tunes BIND's own recursive cache rather than adding a second cache; the separate dnsdist packet cache (`packaging/dnsdist.conf`, `newPacketCache(100000, {maxTTL=86400, minTTL=0, temporaryFailureTTL=60, staleTTL=60})`) remains a fixed packaging-time constant and is a known limitation once per-client policy is enforced (see `docs/progress.md`). |
...
| Scheduled/automatic backups | Not present in AdGuard either | systemd timer with configurable interval and retention-count pruning | `packaging/bindguard-backup.timer`/`.service`, `app/backup.py::deploy_backup_schedule` | complete | none — `tests/test_backup.py::test_prune_backups_keeps_only_newest_n` | |
| Configuration import/export via UI or API | Not present in AdGuard's API either (config-file-only) | Import and Migration page: AdGuard Home migration (YAML upload or read-only API connection), Pi-hole text/list import, BindGuard-native JSON export/import, plus CSV/XLSX/hosts/BIND-zone/BindGuard-CSV import for Local DNS, with column mapping, preview, conflict resolution, backup, and rollback | `app/importer.py`, `web/templates/import_migration.html` (`/import`) | complete | none — `tests/test_importer.py` | BindGuard now goes further than AdGuard here (which has no import/export API at all): a real translation layer for filters/rewrites/clients/upstream resolvers, not just Local DNS CSV. Full appliance backup/restore remains `scripts/backup.sh`/`restore.sh` and the native Backup page. |
| Replication / multi-node sync | Not an AdGuard feature | BindGuard-native one-way primary-to-replica sync: token enrollment, mTLS client certs, generation hashes, allowlisted table payloads, replica rollback on failed deploy, drift check, and revoked-peer enforcement | `app/replication.py`, `web/templates/replication.html` (`/replication`) | complete | none — `tests/test_replication.py` plus live `/tmp/replica-test` verification | It intentionally is not automatic failover or bidirectional sync; promotion remains a deliberate manual action. |
