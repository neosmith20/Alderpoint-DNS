...
- Scheduled backups use `bindguard-backup.timer`.
...
4. BindGuard takes a safety backup before applying.
...
- Use `systemctl status named dnsdist bindguard bindguard-analytics`.
- Use `bindguard-diagnostics --output-dir /tmp` for a sanitized support
  bundle.
