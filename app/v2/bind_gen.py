...
def render_named_conf(
    ...
    # reject an empty forwarders list outright, so "zero configured
    # forwarders means BIND operates as a normal recursive resolver
    # using the DNS root and authoritative hierarchy" had no real
    # generation path at all -- the only way to reach zero forwarders
    # (disabling/deleting every managed upstream) was itself unbuilt
    # until this same pass. An empty list omits `forward only;` and the
    # `forwarders {}` clause entirely -- with `recursion yes;` already
    # set above, that is BIND's genuine, unmodified default behavior:
    # real iterative resolution from the root hints down, never a
    # substituted third-party resolver.
    ...
    if forwarders:
        ...
        forward_lines = ["\tforward only;", "\tforwarders { " + forwarders_clause + "; };"]
...
class BindContext:
    ...
    def __post_init__(self) -> None:
        # Real defect fixed here (owner-reported, "Zero Managed
        # Upstreams" locked decision): this used to reject any context
        # with no forwarders outright, which meant native recursive/root
        # resolution (BIND's own default behavior with no `forwarders`
        # clause -- the exact "zero managed upstreams" decision the
        # roadmap already locks in) had no real code path at all. Empty
        # forwarders is now a valid, intentional context shape meaning
        # exactly that; see render_named_conf's own forwarders=[]
        # handling for the generated named.conf shape.
