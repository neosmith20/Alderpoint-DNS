...
def _job_is_historical(job: dict[str, Any], last_checked_at: str | None) -> bool:
    ...
    This is what turns dns2's field scenario -- an old failed manual
    Update Job from 2026-08-11, still the only row in
    software_update_jobs, sitting untouched while a fresh, successful
    Check for Updates on 2026-08-14 discovers v1.1.0 -- into "Previous
    Update Job #4, failed on 2026-08-11" instead of a current-looking
    failure. A job with no newer check since it finished (e.g. an install
    that just failed moments ago) is never historical: it is still the
